...
WHERE
	Customer_id IN (
		...
		FROM
			Payment
		WHERE
			Payment_date BETWEEN '2023-08-01'
			AND '2023-08-10'
	);
...
-- dates.
CREATE
OR REPLACE FUNCTION increase_salary_for_hr_employee(employee_id INT) RETURNS VOID AS $ $ BEGIN -- Declare variables to store the employee's current salary and department
...
-- (B)
CREATE
OR REPLACE FUNCTION count_sundays_between_dates(start_date DATE, end_date DATE) RETURNS INTEGER AS $ $ DECLARE total_sundays INTEGER := 0;
...
DO $ $ DECLARE film_record film % ROWTYPE;
...
-- Customer (Customer_id, first_name, last_name)
CREATE
OR REPLACE FUNCTION payment_delete_trigger() RETURNS TRIGGER AS $ $ BEGIN -- Insert the deleted data into the customer_backup table
INSERT INTO
	customer_backup (first_name, last_name, amount, payment_id)
SELECT
	C.first_name,
...
WHERE
	IC.c_name = 'Ahmedabad Insurance Company';
...
SELECT
	SUM(F.salary) AS total_salary_spent
FROM
	Faculty AS F
	JOIN Department AS D ON F.d_id = D.dep_id
WHERE
	D.d_name = 'IT Engineering Department';
...
-- scenario.
-- 
DO $ $ DECLARE account_balance NUMERIC := 100000;
...
-- price of products.
DO $ $ DECLARE total_price NUMERIC := 0;
...
CREATE
OR REPLACE PROCEDURE increment_employee_salary(employee_id INT) LANGUAGE plpgsql AS $ $ DECLARE current_salary NUMERIC;
...
CREATE
OR REPLACE FUNCTION check_book_availability(isbn VARCHAR(13)) RETURNS INTEGER AS $ $ DECLARE available_count INTEGER;

BEGIN -- Count the number of available copies of the book with the given ISBN
SELECT
	COUNT(*) INTO available_count
FROM
	Books
WHERE
	books_isbn = isbn;
...
SELECT
	check_book_availability('9780451524935');
...
CREATE
OR REPLACE PROCEDURE update_shipment_status(
	shipment_id INT,
...
INSERT INTO
	status_logs(shipment_id, new_status, update_time)
VALUES
	(shipment_id, new_status, update_time);
...
DO $ $ DECLARE -- Declare variables to hold account details
...
INSERT INTO
	branch_surat (ano, customer_name, balance)
VALUES
	(v_ano, v_customer_name, v_balance);
...
-- ename and salary.
CREATE
OR REPLACE FUNCTION calculate_salary_bonus(
	IN employee_id INT,
	IN bonus_percentage NUMERIC
) RETURNS NUMERIC AS $ $ DECLARE v_salary NUMERIC;
...
CREATE
OR REPLACE PROCEDURE update_employee_salary(
	IN employee_id INT,
	IN new_salary DECIMAL
) LANGUAGE plpgsql AS $ $ BEGIN -- Update the salary of the employee with the provided employee_id
UPDATE
	employee
SET
	salary = new_salary
...
--3
SELECT
	MAX(amount) AS highest_bonus_amount
FROM
	bonus;
...
CREATE
OR REPLACE FUNCTION f_ChkAcctNo(p_acct_no INT) RETURNS INT AS $ $ DECLARE v_count INT;
...
DECLARE
    ...
    CURSOR emp_list(p_employeeno INT, p_job_title VARCHAR) IS
        ...
        AND (p_employeeno IS NULL OR employeeno = p_employeeno)
...
BEGIN
    ...
    LOOP
        ...
        RAISE NOTICE 'Employee: % %', v_firstname, v_lastname;
        RAISE NOTICE 'Hire Date: %', TO_CHAR(v_hiredate, 'DD-MON-YYYY');
...
DO $$
BEGIN
    -- Call the emp_infor procedure with parameters
    emp_infor(123, 'Manager'); -- Replace 123 and 'Manager' with desired employee number and job title
END;
$$;
